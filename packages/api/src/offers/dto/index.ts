import { IsString, IsOptional, IsEnum, IsNumber, IsInt, Min, Max, IsUUID, IsBoolean, IsArray, ValidateNested, IsObject, IsNotEmpty, IsDateString } from 'class-validator';
...
export class PricingTierDto {
  @ApiProperty({
    description: 'Upper limit of units for this tier. Use `null` for the final unlimited tier.',
    example: 100,
    nullable: true,
  })
  @IsNumber()
  ...
  @ApiProperty({
    description: 'Price per unit in smallest currency unit (e.g., cents for USD). Applied to units within this tier.',
    example: 1000,
    minimum: 0,
  })
  @IsInt()
  ...
  @ApiPropertyOptional({
    description: 'Optional flat fee in smallest currency unit added when this tier is reached. Use for "base + per-unit" pricing.',
    example: 5000,
    minimum: 0,
  })
  @IsOptional()
...
- **volume**: All units priced at the tier rate based on total volume (e.g., 100 units = all at $3/unit)`,
  ...
  @ApiProperty({
    description: 'ISO 4217 three-letter currency code. Must match what your billing provider supports.',
    example: 'USD',
    minLength: 3,
    maxLength: 3,
  })
  ...
  @ApiProperty({
    description: 'Price amount in smallest currency unit (e.g., cents for USD, pence for GBP). For tiered/volume pricing, this is ignored in favor of `tiers`.',
    example: 2900,
    minimum: 0,
  })
  @IsInt()
  ...
  @ApiPropertyOptional({
    description: 'Billing interval for recurring subscriptions. Omit for one-time purchases.',
  ...
  @ApiPropertyOptional({
    description: 'Number of intervals between billings. For example, `interval: "month"` with `intervalCount: 3` bills quarterly. Default: 1.',
    example: 1,
    minimum: 1,
    maximum: 12,
    default: 1,
...
- **metered**: Charge based on reported usage at end of billing period`,
  ...
  @ApiPropertyOptional({
    description: 'Pricing tiers for tiered/volume pricing models. Required when `model` is "tiered" or "volume". Last tier should have `upTo: null`.',
    type: [PricingTierDto],
    example: [
...
export class TrialConfigDto {
  @ApiProperty({
    description: 'Length of trial period in days. Customer will not be charged during this period.',
    example: 14,
    minimum: 1,
    maximum: 365,
  })
  ...
  @ApiProperty({
    description: 'If true, customer must provide payment method to start trial (reduces fraud, enables auto-conversion). If false, trial can start without payment info.',
    example: true,
  })
  @IsBoolean()
...
export class EntitlementConfigDto {
  @ApiProperty({
    description: 'Unique identifier for the feature this entitlement grants access to. Use consistent naming across offers (e.g., "api_calls", "seats", "storage_gb").',
    example: 'api_calls',
  })
  @IsString()
  featureKey!: string;
...
export class OfferConfigDto {
  @ApiProperty({
    description: 'Pricing configuration defining how much and how often to charge.',
  ...
  @ApiPropertyOptional({
    description: 'Optional trial period configuration. When set, new subscribers start with a free trial before being charged.',
    type: TrialConfigDto,
  })
  @IsOptional()
  @ValidateNested()
  ...
  @ApiProperty({
    description: 'List of entitlements (features/quotas) granted to subscribers of this offer. These are provisioned when a subscription becomes active and revoked when it ends.',
  ...
  @ApiPropertyOptional({
    description: 'Arbitrary key-value metadata stored with the offer. Useful for internal tracking, feature flags, or integration data.',
  ...
  @ApiPropertyOptional({
    description: 'Escape hatch for advanced billing provider configuration not covered by standard fields. Passed directly to the provider during sync. Use sparingly.',
  })
  @IsOptional()
  @IsObject()
...
export class CreateOfferRequestDto {
  @ApiProperty({
    description: 'Human-readable name for the offer. Displayed in admin dashboards and may be shown to customers.',
    example: 'Pro Plan',
    minLength: 1,
    maxLength: 200,
  })
  ...
  @ApiPropertyOptional({
    description: 'Optional description explaining the offer. Useful for distinguishing similar offers.',
...
export class CreateVersionRequestDto {
  @ApiProperty({
    description: 'New configuration for this version. Changes take effect only after publishing. Existing subscribers remain on their current version.',
    type: OfferConfigDto,
  })
  @ValidateNested()
  @Type(() => OfferConfigDto)
...
export class UpdateOfferDto {
  @ApiPropertyOptional({
    description: 'Updated offer name. Changes apply immediately to the offer (not version-specific).',
...
export class PublishOfferDto {
  @ApiPropertyOptional({
    description: 'UUID of the specific version to publish. If omitted, publishes the latest draft version. Only draft versions can be published.',
...
export class RollbackOfferDto {
  @ApiProperty({
    description: 'UUID of a previously published version to use as the basis for a new draft. This creates a new draft version with the same config as the target version.',
    example: '123e4567-e89b-12d3-a456-426614174000',
    format: 'uuid',
  })
  @IsUUID()
...
export class QueryOffersDto {
  ...
  @ApiPropertyOptional({
    description: 'Pagination cursor from a previous response. Pass `nextCursor` from the last response to get the next page.',
    example: '123e4567-e89b-12d3-a456-426614174000',
  })
  @IsOptional()
  @IsString()
