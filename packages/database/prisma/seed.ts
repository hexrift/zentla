...
const API_KEY_SECRET = process.env.API_KEY_SECRET || 'your-32-character-secret-key-here';
...
function generateApiKey(environment: 'live' | 'test'): {
  ...
  const hash = crypto.createHash('sha256').update(fullKey).update(API_KEY_SECRET).digest('hex');
