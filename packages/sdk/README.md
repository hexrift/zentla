...
const hasPremium = entitlements.find(e => e.featureKey === 'premium')?.hasAccess;
...
const newOffer = await relay.offers.create({
  ...
  config: {
    pricing: { model: 'flat', currency: 'USD', amount: 2900, interval: 'month' },
...
```typescript
// List customers
const { data: customers } = await relay.customers.list({ email: 'user@example.com' });
...
const { entitlements, activeSubscriptionIds } = await relay.customers.getEntitlements('cust_123');
